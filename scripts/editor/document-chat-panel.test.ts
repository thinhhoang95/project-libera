...
test("chat captures both editors, sends the draft, and restores saved conversations", async () => {
  ...
  try {
    ...
    await act(async () => { const history = host.querySelector<HTMLSelectElement>("select")!; history.value = firstId; history.dispatchEvent(new dom.window.Event("change", { bubbles: true })); });
    assert.ok(host.textContent?.includes("A helpful answer"));
