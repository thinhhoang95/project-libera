...
test("chat authenticates, validates input, and uses configured Preferences for initial and regenerated responses", async () => {
  ...
  globalThis.fetch = async (_url, init) => {
    ...
    systemInstructions.push(payload.messages[0].content);
  ...
  try {
    ...
    const regeneratedResponse = await POST(request({ reasoningEffort: "xhigh", messages: [{ id: "1", role: "user", text: "Explain", contexts: [{ kind: "document", path: "a.md", name: "a.md", text: "Unsaved content" }] }] }));
    assert.equal(regeneratedResponse.status, 200);
    assert.equal(calls, 2);
    assert.equal(systemInstructions.length, 2);
    assert.ok(systemInstructions.every((instruction) => instruction.includes("Use a two-sentence maximum.")));
  } finally {
