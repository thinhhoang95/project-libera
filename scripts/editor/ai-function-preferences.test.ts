...
import { POST as imageToMarkdown } from "../../src/app/api/ai-image-to-markdown/route";
...
const functions = ["formatting", "rewrite", "chat", "imageToMarkdown", "latex"] as const;
const environmentNames = { formatting: "FORMATTING", rewrite: "REWRITE", chat: "CHAT", imageToMarkdown: "IMAGE_TO_MARKDOWN", latex: "LATEX" } as const;

test("each AI endpoint uses its own model and effort; chat overrides remain local", async () => {
  const keys = ["OPENROUTER_API_KEY", "LIBERA_OPENROUTER_MODEL", "LIBERA_AI_REWRITE_CUSTOM_INSTRUCTION", ...functions.flatMap((name) => [`LIBERA_AI_${environmentNames[name]}_MODEL`, `LIBERA_AI_${environmentNames[name]}_REASONING_EFFORT`])];
  const original = Object.fromEntries(keys.map((key) => [key, process.env[key]]));
  const originalFetch = globalThis.fetch;
  const efforts = { formatting: "low", rewrite: "high", chat: "xhigh", imageToMarkdown: "medium", latex: "max" } as const;
  process.env.OPENROUTER_API_KEY = "test-key";
  process.env.LIBERA_OPENROUTER_MODEL = "legacy/model";
  process.env.LIBERA_AI_REWRITE_CUSTOM_INSTRUCTION = "Keep the author's distinctive voice.";
  for (const name of functions) {
    process.env[`LIBERA_AI_${environmentNames[name]}_MODEL`] = `custom/${name}`;
  ...
  try {
    ...
    assert.match(calls.at(-1)?.messages[0]?.content ?? "", /Keep the author's distinctive voice\./);
    assert.equal((await imageToMarkdown(request({ imageSource: "data:image/png;base64,AA==" }))).status, 200);
    ...
    assert.deepEqual(calls.map(({ model, reasoning }) => [model, reasoning.effort]), [
      ["custom/formatting", "low"], ["custom/rewrite", "high"], ["custom/imageToMarkdown", "medium"], ["custom/chat", "xhigh"], ["custom/chat", "medium"],
    ]);
    let parts = 0;
