...
test("each AI endpoint uses its own model and effort; chat overrides remain local", async () => {
  const keys = ["OPENROUTER_API_KEY", "LIBERA_OPENROUTER_MODEL", ...functions.flatMap((name) => [`LIBERA_AI_${environmentNames[name]}_MODEL`, `LIBERA_AI_${environmentNames[name]}_REASONING_EFFORT`])];
